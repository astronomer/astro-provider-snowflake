...
from astronomer.providers.snowflake.decorators.snowpark import (
    ...
    snowpark_containers_python_task
)
...
def snowpark_test_dag():
    ...
    @task.snowpark_ext_python(task_id='EPdec', python=_SNOWPARK_BIN, temp_data_schema=_TEST_SCHEMA)
    def epdec(rawdf):
    ...
    @task.snowpark_virtualenv(task_id='VEdec', python_version='3.9', requirements=_PACKAGES, database=_TEST_DATABASE, temp_data_output='table')
    ...
    def spop(newdf):
        return newdf #['VENDOR_ID']
    
    SPop = SnowparkPythonOperator(task_id='SPop', 
    ...
    def veop(df):
        import pandas as pd
        print('########')
        #TODO: Why is this rendering as a string?
        print(type(df))
        print(df[0])
        return df #pd.concat([df_series[0], df_series[1]], axis=1)
    
    VEop = SnowparkVirtualenvOperator(task_id='VEop', 
    ...
    EPop = SnowparkExternalPythonOperator(task_id='EPop',
    ...
    def check_runner_status(service_name:str):
        ...
        if _LOCAL_MODE:
            ...
            service_urls, service_public_headers = hook.get_service_urls(service_name=service_name)
        ...
        response = requests.get(url=runner_conn['url'].split('/task')[0], headers=runner_conn['headers'])
    ...
    SCSop = SnowparkContainersPythonOperator(task_id='SCSop',
                                             python_callable=scsop,
                                             op_args = [raw_table])
