...
from api.log import MessageType, log
...
async def websocket_endpoint(websocket: WebSocket) -> None:
    ...
    try:
        ...
        asyncio.create_task(
            websocket.send_json(
                log(MessageType.EXEC, "Checking user access with Snowflake.")
            )
        )

        # check auth.  If the user can't authenticate to Snowflake we won't accept the run.
        response = (
            SnowparkSession.builder.configs(payload["snowflake_user_conn_params"])
            .create()
            .sql("select 1")
            .collect()
        )
        ...
        if str(response) != "[Row(1=1)]":
            await asyncio.create_task(
                websocket.send_json(json.dumps(log(MessageType.ERROR, response[1])))
            )
            return

        elif payload.get("python_callable_str"):
            with PythonProjectEnv(
                run_dir=run_dir,
                ...
                asyncio.create_task(
                    websocket.send_json(
                        log(MessageType.EXEC, "Running python callable.")
                    )
                )
                ...
                try:
                    ...
                    await asyncio.create_task(
                        websocket.send_json(log(MessageType.RESULT, parsed))
                    )
                except:
                    await asyncio.create_task(
                        websocket.send_json(
                            log(
                                MessageType.ERROR,
                                f"Cannot return data type {str(type(ret_val))}",
                            )
                        )
                    )
        ...
        await asyncio.create_task(
            websocket.send_json(log(MessageType.ERROR, traceback.format_exc()))
        )
