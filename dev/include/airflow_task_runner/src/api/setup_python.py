...
class PythonProjectEnv:
    ...
    def __enter__(self):
        ...
        assert not self.project_dir.exists(), "Project prefix collision. Retry task."
        self.project_dir.mkdir()

        self.payload_file.write_text(json.dumps(self.payload))

        print("Creating a python virtual environment.")
        self.python_exe = self.payload.get(
            Path(f"/usr/local/bin/python{self.payload['python_version']}"),
            Path(sys.executable),
        )
        assert (
            self.python_exe.is_absolute() and self.python_exe.exists()
        ), f"User provided python {self.payload['python_version']} not available on runner.  Specify maj.min version only."

        virtualenv_cmd = [
            self.python_exe.as_posix(),
            "-m",
            "virtualenv",
        ...
        subprocess.check_call(
            virtualenv_cmd, stderr=subprocess.STDOUT, stdout=subprocess.PIPE
        ...
        if self.payload["requirements"]:
            ...
            pip_cmd = (
                [self.user_python, "-m", "pip", "install", "--cache-dir=/tmp/pip_cache"]
                + self.payload.get("pip_install_options")
                + self.payload["requirements"]
            )

            subprocess.check_call(
                pip_cmd, stderr=subprocess.STDOUT, stdout=subprocess.PIPE
            )  # , log_file=self.log_file)
        ...
        if self.payload["op_args"] or self.payload["op_kwargs"]:
            ...
            self.input_path.write_bytes(
                pickle.dumps(
                    {
                        "args": self.payload["op_args"],
                        "kwargs": self.payload["op_kwargs"],
                    }
                )
            )
        ...
        try:
            subprocess.check_call(
                ["useradd", "-d", self.project_dir.as_posix(), self.project_prefix],
                stderr=subprocess.STDOUT,
                stdout=subprocess.PIPE,
            )  # , log_file=self.log_file)
        except subprocess.CalledProcessError:  # User exists
            raise Exception(
                "Project user exists. Project prefix collision. Retry task."
            )
