...
def _is_table_arg(arg: Any):
    if ((Table or TempTable) and isinstance(arg, (Table, TempTable))) or (
        SnowparkTable and isinstance(arg, SnowparkTable)
    ):
        arg = arg.to_json()

    if isinstance(arg, dict) and arg.get("class", "") in [
        "SnowparkTable",
        "Table",
        "TempTable",
    ]:
...
def _deserialize_snowpark_args(
    ...
    if uri:
        if uri["xcom_stage"]:
            ...
            raise Exception("Failed to parse SnowparkTable URI.")
...
def _serialize_table_args(arg: Any):
    ...
    """
    if ((Table or TempTable) and isinstance(arg, (Table, TempTable))) or isinstance(
        arg, SnowparkTable
    ):
        return arg.to_json()
