...
def get_snowflake_xcom_objects() -> dict:
    try:
        ...
        raise AirflowException(
            "AIRFLOW__CORE__XCOM_SNOWFLAKE_CONN_NAME environment variable not set."
        )
    ...
    try:
        ...
        raise AirflowException(
            "AIRFLOW__CORE__XCOM_SNOWFLAKE_TABLE environment variable not set"
        )

    assert (
        len(snowflake_xcom_table.split(".")) == 3
    ), "AIRFLOW__CORE__XCOM_SNOWFLAKE_TABLE is not a fully-qualified Snowflake table object"

    try:
        snowflake_xcom_stage = os.environ["AIRFLOW__CORE__XCOM_SNOWFLAKE_STAGE"]
    except:
        raise AirflowException(
            "AIRFLOW__CORE__XCOM_SNOWFLAKE_STAGE environment variable not set"
        )

    assert (
        len(snowflake_xcom_stage.split(".")) == 3
    ), "AIRFLOW__CORE__XCOM_SNOWFLAKE_STAGE is not a fully-qualified Snowflake stage object"

    return {
        "conn_id": snowflake_conn_id,
        "table": snowflake_xcom_table,
        "stage": snowflake_xcom_stage,
    }
...
def check_xcom_conn(snowflake_conn_id: str) -> str:
    ...
    assert (
        response[0] is True
    ), f"Snowflake XCOM connection {snowflake_conn_id} error. {response[1]}"
...
def check_xcom_table(snowflake_conn_id: str, snowflake_xcom_table: str) -> str:
    ...
    xcom_table_schema = SnowflakeHook(snowflake_conn_id=snowflake_conn_id).get_records(
        f"DESCRIBE TABLE {snowflake_xcom_table}"
    )

    assert (
        xcom_table_schema == expected_schema
    ), f"""
...
def check_xcom_stage(snowflake_conn_id: str, snowflake_xcom_stage: str) -> str:
    try:
        SnowflakeHook(snowflake_conn_id=snowflake_conn_id).get_records(
            f"DESCRIBE STAGE {snowflake_xcom_stage}"
        )
