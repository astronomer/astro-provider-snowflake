...
class SnowparkContainersHook(SnowflakeHook):
    ...
    def create_pool(
        ...
        if gpu_name and gpu_name.upper() not in self.gpu_types:
            raise AttributeError(
                f"Unsupported option {gpu_name} specified for gpu_name."
            )

        if instance_family and instance_family.upper() not in self.instance_types:
            raise AttributeError(
                f"Unsupported option {instance_family} specified for instance_family."
            )
        ...
        if gpu_name:
            if instance_family not in ["standard_1", "standard_2", "standard_3"]:
                raise AttributeError(
                    "Invalid combination of instance_family and gpu_name."
                )
            else:
                gpu_option_str = f" GPU_OPTIONS = ( accelerator = {gpu_name} ) "

        replace_existing_str = (
            "OR REPLACE COMPUTE POOL"
            if replace_existing
            else "COMPUTE POOL IF NOT EXISTS"
        )

        response = self.get_pandas_df(
            f"""CREATE {replace_existing_str} {pool_name}
                                            MIN_NODES = {min_nodes}
                                            MAX_NODES = {max_nodes}
        ...
        if "fail" in response["status"][0]:
            raise AirflowException(
                f"Create service failed. Status {response['status'][0]}"
            )
        else:
            return pool_name
    ...
    def list_pools(
        self, name_prefix: str = None, pool_name: str = None, limit: int = None
    ) -> dict or None:
        """
        ...
        response = self.get_pandas_df(
            f"SHOW COMPUTE POOLS {like_str} {prefix_str} {limit_str};"
        )
    ...
    def remove_pool(self, pool_name: str):
        ...
        if self.list_pools(pool_name=pool_name)[pool_name.upper()]["num_services"] > 0:
            raise AirflowException(
                "All services must be stopped before removing a compute pool."
            )
        else:
    ...
    def create_repository(
        ...
        assert (
            database and schema
        ), "Database and schema must be set in conn params, hook params or args."

        replace_existing_str = (
            "OR REPLACE IMAGE REPOSITORY"
            if replace_existing
            else "IMAGE REPOSITORY IF NOT EXISTS"
        )

        self.get_pandas_df(
            f"""CREATE {replace_existing_str} {database}.{schema}.{repository_name};"""
        )
    ...
    def list_repositories(
        ...
        response = self.get_pandas_df(
            f"SHOW IMAGE REPOSITORIES {like_str} {prefix_str} {limit_str};"
        )
        response = response.set_index("name").to_dict("index")
    ...
    def get_repo_url(
        ...
        assert (
            len(account.split("-")) >= 2
        ), "For Snowpark Container registry operations the account must be listed with '<org_name>-<account_name>.  Account name may contain dashes or underscores."

        repository_url = self.list_repositories(
            repository_name=repository_name, database=database, schema=schema
        )[repository_name.upper()]["repository_url"]
    ...
    def remove_repository(
        ...
        self.get_pandas_df(
            f"""DROP IMAGE REPOSITORY {database}.{schema}.{repository_name};"""
        )
    ...
    def create_service(
        ...
        :type spec_file: str
        :param repository_name: Name of the Snowpark Container repository where the image has been uploaded.
        ...
        assert (
            self.database and self.schema
        ), "Database and schema must be set in conn params, hook params or args."

        repository_url = self.get_repo_url(
            repository_name=repository_name, database=self.database, schema=self.schema
        )
        ...
        for service_name, service_spec in spec.items():
            for container in service_spec["spec"]["container"]:
                container["image"] = (
                    repository_url + "/" + container["image"].split("/")[-1]
                )
        ...
        with tempfile.NamedTemporaryFile(
            mode="w+", dir=os.getcwd(), suffix="_spec.yaml"
        ) as tf:
            ...
            try:
                self.run(
                    f"""CREATE TEMPORARY STAGE {temp_stage_name};
                ...
                if "already exists" in e.msg:
                    raise AirflowException(
                        f"Service {service_name.upper()} exists.  The service must be removed before attempting creation."
                    )
    ...
    def list_services(
        ...
        services = (
            self.get_pandas_df(f"SHOW SERVICES {like_str} {prefix_str} {limit_str};")
            .set_index("name")
            .to_dict("index")
        )
        ...
        for service, service_detail in services.items():
            ...
            service_detail["public_endpoints"] = {
                k: f"https://{v}" for k, v in endpoints.items()
            }

            service_status = self.get_first(
                f"call system$get_service_status('{self.database}.{self.schema}.{service.upper()}', 10);"
            )
            services[service]["container_status"] = {}
            for container in json.loads(service_status[0]):
                service_detail.setdefault("container_status").update(
                    {container.pop("containerName"): container}
                )
        ...
        if quiet:
            ...
            for service, service_status in services.items():
                status[service] = {}
                for container, container_status in service_status[
                    "container_status"
                ].items():
    ...
    def get_service_logs(
        ...
        try:
            status = self.list_services(service_name=service_name)[
                service_name.upper()
            ]["container_status"]
        except KeyError:
            raise AirflowException(
                f"Service '{service_name} is not running.  Cannot fetch logs."
            )
        logs = {}
        for container, container_status in status.items():
            instance_id = container_status["instanceId"]
            instance_logs = self.get_records(
                f"CALL SYSTEM$GET_SERVICE_LOGS('{self.database}.{self.schema}.{service_name}', {instance_id}, '{container}');"
            )
    ...
    def push_images(
        ...
        :param spec_file_name: Path to an existing YAML specification for the service
        :type spec_file: str
        :param repository_name: Name of the Snowpark Container repository where the image will be pushed.
            Optional in local mode.
        :type repository_name: str
        ...
        if not image_sources:
            ...
            try:
                ...
                for container in spec["container"]:
                    image_sources[container["name"]] = {
                        "image_source": container["image"]
                    }
            except:
                raise AttributeError(
                    "No image source provided and no image found in docker compose specs."
                )
        else:
            temp_sources = {}
        ...
        for container, images in image_sources.items():
            ...
            if image and image.attrs["Architecture"] == "amd64":
                ...
                image = docker_pull(
                    image_source=images["image_source"], platform="linux/amd64"
                )

            print(f"Pushing {image.id} to {images['image_dest']}:{images['tag']}")
            docker_push(
                image_source=image,
                image_dest=images["image_dest"],
                tag=images["tag"],
